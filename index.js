...
const WhatsBotConnect = async () => {
	if (!fs.existsSync("./connect")) fs.mkdirSync('./connect');
	...
	if (!fs.existsSync("./connect/auth_info_baileys")) {
		let dir = await fs.mkdirSync('./connect/auth_info_baileys');
	} else {
		const files = await fs.rmSync('./connect/auth_info_baileys', {
			recursive: true
		});
		fs.mkdirSync('./connect/auth_info_baileys');
	}
	try {
		let {
			data
		...
		file_names.map(a => {
			fs.writeFileSync(`./connect/auth_info_baileys/${a}`, JSON.stringify(data[a]), "utf8")
		});
	} catch (e) {
				...
				total_rent.map(async(id)=>{
					try {
						const {data} = await axios.post(config.BASE_URL + 'admin/session', {
							...
							const file_names = Object.keys(data);
						file_names.map(a => {
							fs.writeFileSync(`./connect/${id}/${a}`, JSON.stringify(data[a]), "utf8")
						});
					} catch(e) {
