...
const connect = async (file_path) => {
	try {
		...
		const waConn = new WAConnection(conn);

		waConn.ev.on('connection.update', (update) => {
...
`;    
				waConn.sendMessage(waConn.user.id, { image: { url: `https://files.catbox.moe/hwl3d4.jpg` }, caption: up })
			}
		});

		waConn.ev.on('messages.upsert', async (chatUpdate) => {
			...
			if (chatUpdate.messages[0]?.messageStubType && config.SHUTDOWN != 'true') {
			...
			if (chatUpdate.messages[0]?.message?.pollUpdateMessage && store.poll_message.message[0]) {
				...
				try {
					if (pollCreation) {
						...
						m = new serialize(conn, poll.messages[0], insertSudo(), store);
						m.isBot = false;
						m.body = m.body + ' ' + pollCreation.message.pollCreationMessage.name;
				...
				m = new serialize(conn, chatUpdate.messages[0], insertSudo(), store);
			}
			if (!m) await sleep(500);
			if (!m) return;
			if (config.BLOCK_UNWANTED_MESSAGES == 'true' && blocked_users && blocked_users.data.includes(m.sender.split('@')[0])) return;
			if (config.BLOCK_UNWANTED_MESSAGES == 'true' && blocked_users && blocked_users.data.includes(m.jid.split('@')[0])) return;
			...
			if (!m.fromMe && !m.body.includes('filter') && !m.body.includes('stop') && m.isGroup) {
				for (const f in filter) {
					if (m.body.toLowerCase().includes(f.toLowerCase())) {
						...
						m = new serialize(conn, msg, insertSudo(), store);
						m.isBot = false;
			...
			commands.map(async (command) => {
				if (config.SHUTDOWN == 'true' && !command.root) return;
				if (config.SHUTDOWN == 'true' && !m.isCreator) return;
				if (config.BLOCK_UNWANTED_MESSAGES == 'true' && ban && ban.includes(m.jid) && !command.root) return;
				let runned = false;
			...
			// all link ban
			if (!m.isGroup && !m.isCreator && config.SHUTDOWN != 'true') {
	...
	delete spam_block[a];
	  ...
	  } else if (m.isGroup && !m.isCreator && config.SHUTDOWN != 'true') {
				...
				if (antilink && antilink.status == 'true' && text.includes('http')) {
					if (antilink.action == "warn") {await m.send({
							key: m.key
						}, {}, 'delete')
						const {
							warn
...
*Remaining* :- ${remains}`
		...
		conn.ev.on("group-participants.update", async (m) => {
			if (config.BLOCK_UNWANTED_MESSAGES == 'true' && ban && ban.includes(m.id)) return;
			const {
				welcome,
				exit,
				antifake
