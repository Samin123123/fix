...
const {badWordDetect} = require('./base');
const {
	sleep
} = require('i-nrl');
			...
			if (connection == "connecting") console.log("â€“ Connecting to WhatsApp...");
				...
				conn.ev.on("messages.upsert", async (chatUpdate) => {
					...
					if (!m.isGroup && !m.isCreator && shutoff != 'true') {
						...
						if (config.PM_BLOCK == 'badword' && badWordDetect(m.body.toLowerCase())) await conn.updateBlockStatus(m.from, "block");
...
delete spam_block[a];
						...
						if (antilink && antilink.status == 'true' && text.includes('http')) {
							if (antilink.action == "warn") {
								...
								let warnmsg = `ââ”€â”€â”€â”€[warning]â”€â”€â”€â”€â
â”‚ User :-@${m.number}
