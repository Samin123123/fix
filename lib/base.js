...
const {cutAudio,cutVideo,toAudio,toPTT,isAdmin,isBotAdmin,getCompo,getDate,parsedJid,PREFIX,mode,extractUrlsFromString,getJson,isIgUrl,getUrl,isNumber} = require('./handler');
...
async function send_menu(m, version, obj) {
	const image = MediaUrl(obj.BOT_INFO)
		if (image) {
			img_url = image[0];
			theam = img_url.video ? 'video' : 'image';
			obj.BOT_INFO = obj.BOT_INFO.replace(img_url[theam], '').trim();
		}
	const info_vars = obj.BOT_INFO.split(/[;,|]/);
